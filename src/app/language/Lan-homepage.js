const english = {
  ZTLThai: "ZAIMAH TECHNOLOGIES  ",

  heroParagraph:
    "A leading force in the technology industry, driving innovation and excellence across various domains.",
  heroParagraph2:
    "  we are committed to delivering cutting edge solutions in software development services.",
  getStarted: "Get Started",
  seeMore: "See More",
  contactUs: "Contact Us",

  productSec: {
    title: "How we bring tangible business results",
    subtitle:
      "Our homegrown SaaS products are built on state of the art technology, Do you wanna try?",
    Product1: {
      name: "HR BEE",
      description:
        "We help businesses to grow and scale by leveraging technology and digital solutions.",
      url: "https://hrbee.xyz/",
    },
    Product2: {
      name: "Inventory Pro",
      description:
        "We help businesses to grow and scale by leveraging technology and digital solutions.",
      url: "http://inventorypro.xyz/",
    },
    Product3: {
      name: "Sinewy Care",
      description:
        "We help businesses to grow and scale by leveraging technology and digital solutions.",
      url: "http://sinewycare.com/",
    },
    Product4: {
      name: "Apart Pro",
      description:
        "We help businesses to grow and scale by leveraging technology and digital solutions.",
      url: "https://www.apartpro.xyz/",
    },
  },
  serviceSec: {
    title: "Our Services",
    subtitle:
      "We provide a wide range of services to help businesses grow and scale.",
  },
  tabsSec: {
    title: "Office of ZTL ",
    subtitle:
      "We follow a systematic process to ensure that we deliver the best results.",
    Bangladesh: {
      title: "ZTL - in Bangladesh",
      subtitle:
        "At ZTL, we pride ourselves on delivering cutting-edge technology solutions tailored to meet our clients' needs. Our commitment to innovation and excellence has earned us a reputation as a trusted partner in the IT industry. We specialize in a wide range of services, including software development, IT consulting, and system integration.",

      Operations: "Total Operations: 10k+",
      Clients: "Total Clients",
      TeamMembers: "Team Members: 15+",
      Projects: "Projects Completed: 500+",
      Industries: "Industries Served: 20+",
      Years: "Years in Business: 8+",
      Expertise: "Our Expertise: ",
      OurExpertise: [
        "Custom Software Development",
        "Web and Mobile Applications",
        "IT Consulting and Strategy",
        "System Integration",
        "Cloud Solutions",
      ],
    },
  },
  clientsSec: {
    title: "Trusted by over 100+ clients Zaimah Tech worldwide",
  },
  contactSec: {
    title: "Start a Conversation",
    subtitle:
      "Connect with our specialists to develop innovative strategies for your digital products and drive real business value.",
    button: "Contact Us",
  },
};

const thai = {
  ZTLThai: "ZAIMAH TECHNOLOGIES  ",
  heroParagraph:
    "ผู้นำที่สำคัญในอุตสาหกรรมเทคโนโลยี ขับเคลื่อนนวัตกรรมและความเป็นเลิศในหลายสาขา",
  heroParagraph2: "เรามุ่งมั่นที่จะนำเสนอโซลูชั่นล้ำสมัยในบริการพัฒนาซอฟต์แวร์",
  sub1: "เราใช้เครื่องมือดิจิทัล แพลตฟอร์ม และกลยุทธ์ต่าง ๆ เพื่อเพิ่มประสิทธิภาพ ผลผลิต นวัตกรรม และความสามารถในการแข่งขันโดยรวม",
  sub2: "นวัตกรรมดิจิทัลเพื่อการเติบโต",
  sub3: "เปลี่ยนแนวคิดให้เป็นความจริง",
  sub4: "มอบความเป็นเลิศในทุกโครงการ",
  heroParagraph:
    "เราใช้เครื่องมือดิจิทัล แพลตฟอร์ม และกลยุทธ์ต่าง ๆ เพื่อเพิ่มประสิทธิภาพ ผลผลิต นวัตกรรม และความสามารถในการแข่งขันโดยรวม",
  getStarted: "มาเริ่มกันเลย",
  seeMore: "ดูเพิ่มเติม",
  contactUs: "ติดต่อเรา",

  productSec: {
    title: "เรานำผลลัพธ์ที่แท้จริงทางธุรกิจมาได้อย่างไร",
    subtitle: "เราช่วยธุรกิจเติบโตและขยายด้วยการใช้เทคโนโลยีและโซลูชันดิจิทัล",
    Product1: {
      name: "HR BEE",
      description:
        "เราช่วยธุรกิจเติบโตและขยายด้วยการใช้เทคโนโลยีและโซลูชันดิจิทัล",
      url: "https://hrbee.xyz/",
    },
    Product2: {
      name: "Inventory Pro",
      description:
        "เราช่วยธุรกิจเติบโตและขยายด้วยการใช้เทคโนโลยีและโซลูชันดิจิทัล",
      url: "http://inventorypro.xyz/",
    },
    Product3: {
      name: "Sinewy Care",
      description:
        "เราช่วยธุรกิจเติบโตและขยายด้วยการใช้เทคโนโลยีและโซลูชันดิจิทัล",
      url: "http://sinewycare.com/",
    },
    Product4: {
      name: "Apart Pro",
      description:
        "เราช่วยธุรกิจเติบโตและขยายด้วยการใช้เทคโนโลยีและโซลูชันดิจิทัล",
      url: "https://www.apartpro.xyz/",
    },
  },
  serviceSec: {
    title: "บริการของเรา",
    subtitle: "เรามีบริการหลากหลายเพื่อช่วยให้ธุรกิจเติบโตและขยายตัว",
  },
  tabsSec: {
    title: "ช่วยองค์กรเติบโตในโลกดิจิทัลใหม่",
    subtitle:
      "เราปฏิบัติตามกระบวนการเชิงระบบเพื่อให้แน่ใจว่าเราส่งมอบผลลัพธ์ที่ดีที่สุด",
    Bangladesh: {
      title: "ZTL - สำนักงานใหญ่อยู่ในบังกลาเทศ",
      subtitle:
        "ที่ ZTL เราภาคภูมิใจในการมอบโซลูชันเทคโนโลยีที่ล้ำสมัยซึ่งออกแบบมาเพื่อตอบสนองความต้องการของลูกค้า ความมุ่งมั่นของเราต่อการนวัตกรรมและความเป็นเลิศได้ทำให้เราได้รับชื่อเสียงในฐานะพันธมิตรที่เชื่อถือได้ในอุตสาหกรรม IT เรามีความเชี่ยวชาญในบริการที่หลากหลาย รวมถึงการพัฒนาซอฟต์แวร์ การให้คำปรึกษาด้าน IT และการบูรณาการระบบ",

      Operations: "จำนวนการดำเนินงานทั้งหมด: 10k+",
      Clients: "จำนวนลูกค้าทั้งหมด: 300+",
      TeamMembers: "สมาชิกในทีม: 15+",
      Projects: "โครงการที่เสร็จสิ้น: 500+",
      Industries: "อุตสาหกรรมที่ให้บริการ: 20+",
      Years: "ปีในธุรกิจ: 8+",
      Expertise: "ความเชี่ยวชาญของเรา: ",
      OurExpertise: [
        "การพัฒนาซอฟต์แวร์ที่กำหนดเอง",
        "แอปพลิเคชันเว็บและมือถือ",
        "การให้คำปรึกษาและกลยุทธ์ด้าน IT",
        "การบูรณาการระบบ",
        "โซลูชันคลาวด์",
      ],
    },
  },
  clientsSec: {
    title: "ได้รับความไว้วางใจจากลูกค้ามากกว่า 100+ รายทั่วโลก Zaimah Tech",
  },
  contactSec: {
    title: "เริ่มสนทนา",
    subtitle:
      "ติดต่อกับผู้เชี่ยวชาญของเราเพื่อพัฒนากลยุทธ์นวัตกรรมสำหรับผลิตภัณฑ์ดิจิทัลของคุณและสร้างค่าใช้งานจริง",
    button: "ติดต่อเรา",
  },
};

export const homePageLanguage = {
  english,
  thai,
};
